...
static int ionic_dev_cmd_debug_stats(struct ionic_dev *idev, u64 stats_pa,
		u16 count, unsigned long max_seconds)
{
	union ionic_dev_cmd cmd = {
		.debug.opcode = IONIC_CMD_DEBUG,
		.debug.debug_type = IONIC_DEBUG_TYPE_STATS,
		.debug.stats.stats_pa = cpu_to_le64(stats_pa),
		.debug.stats.count = cpu_to_le16(count),
	};
					...
					unsigned int num_bars)
{
	...
	// Device info region
	idev->port_info_sz = ALIGN(sizeof(*idev->port_info), IONIC_ALIGN_SZ);

	idev->port_info = malloc_phys(idev->port_info_sz, IONIC_ALIGN_SZ);
...
int ionic_q_init(struct lif *lif, struct ionic_dev *idev, struct queue *q,
				 unsigned int index, unsigned int num_descs,
			   ...
			   unsigned int align_sz)
{
	...
	*qcq = NULL;
	...
	cq_base = (void *)ALIGN((uintptr_t)q_base + q_size, align_sz);
	cq_base_pa = ALIGN(q_base_pa + q_size, align_sz);
...
static int ionic_qcqs_alloc(struct lif *lif)
{
	...
	pid = 0;
	flags = 0;
	err = ionic_qcq_alloc(lif, IONIC_QTYPE_ADMINQ, 0, flags,
			      ADMINQ_LENGTH,
...
int ionic_lif_alloc(struct ionic *ionic, unsigned int index)
{
	...
	lif->info_sz = ALIGN(sizeof(*lif->info), IONIC_ALIGN_SZ);
...
int ionic_poll_adminq(struct lif *lif, struct ionic_admin_ctx *ctx)
{
	unsigned long wait, max_wait;
...
int ionic_adminq_post_wait(struct lif *lif, struct ionic_admin_ctx *ctx)
{
	...
	return ionic_poll_adminq(lif, ctx);
}
...
static void ionic_qcq_sanitize(struct qcq *qcq, unsigned int align_sz)
{
	unsigned int q_size = ALIGN(qcq->q.num_descs * qcq->q.desc_size,
				    align_sz);
	unsigned int cq_size = ALIGN(qcq->cq.num_descs * qcq->cq.desc_size,
				     align_sz);
 ...
 * Initialize the TXQ.
 * */
int ionic_lif_txq_init(struct lif *lif, struct qcq *qcq)
{
	struct queue *q = &qcq->q;
	struct cq *cq = &qcq->cq;
	struct ionic_dev *idev = &lif->ionic->idev;
	...
	ionic_qcq_sanitize(qcq, IONIC_ALIGN_SZ);
	return 0;
}
 ...
 * Initialize the RXQ.
 * */
int ionic_lif_rxq_init(struct lif *lif, struct qcq *qcq)
{
	struct queue *q = &qcq->q;
	struct cq *cq = &qcq->cq;
	struct ionic_dev *idev = &lif->ionic->idev;
...
void ionic_poll_tx(struct net_device *netdev)
{
	...
	while (color_match(comp->color, txcq->done_color)) {
		...
		// tx_comp_index need to be print in the range of 0-(NTXQ_DESC-1)
		stats->tx_comp_index.cnt = (tx_index_cnt % NTXQ_DESC);
		tx_index_cnt++;
	}
}
 ...
 * Driver debug stats
 */
static void ionic_debug_stats_reset(struct ionic_dbg_stats *stats)
{
	IONIC_DBG_STATS_FLD_INIT(stats, tx_total);
	...
	IONIC_DBG_STATS_FLD_INIT(stats, tx_comp_index);
}

int ionic_debug_stats_init(struct ionic *ionic)
{
	int err = 0;
	int stats_count = sizeof(*ionic->dbg_stats) /
				sizeof(struct ionic_dev_debug_stats);
	dma_addr_t stats_pa = virt_to_phys(ionic->dbg_stats);

	ionic_debug_stats_reset(ionic->dbg_stats);
	err = ionic_dev_cmd_debug_stats(&ionic->idev, stats_pa,
					stats_count, devcmd_timeout);
	if (err) {
		DBG_OPROM_ERR(ionic, "Failed to init debug stats\n");
		goto out;
	}
...
int ionic_debug_stats_alloc(struct ionic *ionic)
{
	struct ionic_dbg_stats *stats;

	ionic->dbg_stats = NULL;
	stats = malloc_phys(sizeof(*ionic->dbg_stats), IONIC_ALIGN_SZ);
	if (!stats) {
		DBG_OPROM_ERR(ionic, "failed to allocate debug stats region\n");
		return -ENOMEM;
	}
	ionic_debug_stats_reset(stats);
	ionic->dbg_stats = stats;

	return 0;
}

void ionic_debug_stats_free(struct ionic *ionic)
{
	if (ionic->dbg_stats)
		free_phys(ionic->dbg_stats, sizeof(*ionic->dbg_stats));
