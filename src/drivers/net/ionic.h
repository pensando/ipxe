...
#define IONIC_DBG_STATS_FLD_INIT(X, FLD) 			\
	do { 							\
		strncpy(X->FLD.fld, #FLD, sizeof(X->FLD.fld));	\
		X->FLD.cnt = 0;					\
	} while (false)

struct ionic_dbg_stats
{
	struct ionic_dev_debug_stats tx_total;
	...
	struct ionic_dev_debug_stats tx_comp_index;
};
...
struct ionic {
	...
	struct ionic_dbg_stats *dbg_stats;
	u16 link_status;
	u8 qs_running;
	u8 fw_running;
	u64 last_eid;
...
// helper functions for DEBUG cmd
